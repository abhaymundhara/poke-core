...
export class AutopilotLiveDaemon {
  ...
  async pollOnce(): Promise<{ web: LiveWebSignalBundle; platform: PlatformSignalBundle; wakeReasons: string[] }> {
    this.pollCount += 1;
    const now = this.deps.clock();
    const [web, platform] = await Promise.all([
      pollLiveWebSignals(this.deps.query, now),
      pollGithubPlatformSignals(this.deps.githubWatches ?? [], now),
    ]);
    this.lastPollAt = now;
    this.lastWebQuery = this.deps.query;
    ...
    if (wakeReasons.length > 0) {
      this.lastWakeAt = now;
      this.deps.onWake?.(wakeReasons[0], { reasons: wakeReasons, webFreshness: web.freshnessScore, platformSignals: platform.sourceCount });
    }
    return { web, platform, wakeReasons };
  }
