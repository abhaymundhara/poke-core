...
import type { ChunkRecord, DocumentLifecycle, RetrievalEvidenceHit, RetrievalHit, RetrievalQuery } from './types';
...
export function scoreChunk(chunk: ChunkRecord, query: RetrievalQuery): RetrievalHit {
  ...
  const queryTokens = querySignals.tokens;
  const queryEmbedding = defaultSemanticEmbeddingModel.embedText(query.query);
  const chunkEmbedding = chunk.embedding.length ? chunk.embedding : defaultSemanticEmbeddingModel.embedText(chunk.text);
  ...
  const lexicalWeight = mode === 'lexical' ? 0.58 : mode === 'semantic' ? 0.2 : 0.32;
  const semanticWeight = mode === 'semantic' ? 0.58 : mode === 'lexical' ? 0.2 : 0.48;
  ...
  const exactPhraseBoost = Math.min(1, phraseMatches.length) * (query.boost?.exactPhrase ?? 0.28);
  const sourceWeight = sourceScore(chunk.source, chunk.lifecycle, query);
  const structural = structuralBoost(chunk, query);
...
export function scoreEvidence(anchor: RetrievalHit, candidate: RetrievalHit, query: RetrievalQuery): RetrievalEvidenceHit {
  const querySignals = extractSemanticSignals(query.query).axes;
  ...
  const semanticBridge = Math.max(0, candidate.semanticScore * 0.5 + cosineSimilarity(defaultSemanticEmbeddingModel.embedText(anchor.excerpt), defaultSemanticEmbeddingModel.embedText(candidate.excerpt)) * 0.35);
