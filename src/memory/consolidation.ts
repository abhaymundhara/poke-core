...
export type ConsolidationInput = { now?: number; workingFacts: MemoryFact[]; episodicItems: EpisodicMemoryItem[]; sourceDocuments?: MemoryDocument[]; decayHalfLifeHours?: number };
...
function extractEntities(text: string): string[] {
  ...
  const ids = lower.match(/\b(?:thread|event|meeting|project|ticket|case|doc|page|task)[-_]?[a-z0-9]+\b/g) ?? [];
...
export class MemoryConsolidationJob {
  ...
  run(): ConsolidationResult {
    ...
    const promotedFacts = factsWithScores.filter((entry) => entry.salience >= 0.5).map(({ fact, salience }) => ({ ...fact, confidence: Number(Math.min(1, Math.max(fact.confidence, salience)).toFixed(3)) }));
    ...
    for (const [documentId, entry] of grouped.entries()) {
      ...
      for (const [position, text] of chunks.entries()) {
        ...
        semanticChunks.push({
          ...
          lifecycle: entry.tags.has('episodic') ? 'thread' : entry.tags.has('semantic') ? 'reference' : 'unknown',
          source: entry.source,
        });
      }
    }
    ...
    return { promotedFacts, semanticDocuments, semanticChunks, links, decayedFacts, summary: `${promotedFacts.length} facts promoted, ${semanticDocuments.length} semantic docs, ${links.length} entity links` };
  }
}
